/**
  ...
  - Supports: X/Y/Z surface, inner/outer corner, rect boss/pocket, circ boss/bore
...
description = "FANUC / GSK 218MC with Probing V6 (O9100-series)";
vendor = "Fanuc / GSK";
...
longDescription = "Generic post for Fanuc. Modified for GSK 218MC with O9100-series custom probe library, two-pass probing, dynamic WCS selection, and Fusion 360 CAM integration.";
...
properties = {
  ...
  EnableZeroPointCompensation: {
    ...
    description: "When enabled, XY probe cycles (boss, bore, pocket, corner) call O9130 after probing to translate the probe WCS result back to the setup WCS (workOffset). Set probeWorkOffset != workOffset in Fusion to use.",
    group      : "probing",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
var settings = {
  ...
  workPlaneMethod: {
    ...
    eulerCalculationMethod: "machine", // ('standard' / 'machine') 'machine' adjusts euler angles to match the machines ABC orientation, machine configuration required
...
function onCycleEnd() {
  if (isProbeOperation()) {
    zOutput.reset();
    gMotionModal.reset();
    // Replace G65 P9810 with a simple G00 rapid move to the retract height
    writeBlock(gMotionModal.format(0), zOutput.format(cycle.retract)); // G00 Z (retract height)
  } else {
...
function onSectionEnd() {
  ...
  if (isProbeOperation()) {
  writeBlock(mFormat.format(49)); // M49 - Turn probe off
...
function defineMachine() {
  var useTCP = false; // TCP not needed for direct A-axis movements
  var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-360, 360], cyclic:true, preference:0, tcp:useTCP});
  ...
  if (!receivedMachineConfiguration) {
    ...
    if (performRewinds) {
      ...
      safeRetractDistance = (unit == MM) ? 1 : 25; // additional distance to retract out of stock, can be overridden with a property
      safeRetractFeed = (unit == MM) ? 20 : 500; // retract feed rate
      safePlungeFeed = (unit == MM) ? 10 : 250; // plunge feed rate
    ...
    if (machineConfiguration.isMultiAxisConfiguration()) {
      machineConfiguration.setMultiAxisFeedrate(
        FEED_FPM, // Use feed per minute for 4th-axis movements
        9999.99, // maximum output value for inverse time feed rates (not used)
        INVERSE_MINUTES, // not used since we're using FPM
...
function getWorkPlaneMachineABC(_section, rotate) {
  ...
  // Skip the strict direction check for 4th-axis setups
...
function writeProbeCycle(cycle, x, y, z, P, F) {
  ...
  // Map Fusion WCS to G-code number for our O9100-series programs
  // Fusion has TWO WCS concepts for probing:
  //   currentSection.workOffset      = Setup WCS (drives positioning, e.g. G54 pallet)
  //   currentSection.probeWorkOffset = Target WCS to UPDATE (e.g. G55 part zero)
  // Our S parameter must be the TARGET, not the driving WCS.
  var fusionWCS;
  if (currentSection.strategy == "probe" && currentSection.probeWorkOffset > 0) {
    fusionWCS = currentSection.probeWorkOffset;
  } else {
    fusionWCS = currentSection.workOffset > 0 ? currentSection.workOffset : 1;
  }
  if (fusionWCS > 6) {
    error(localize("O9100-series probe programs only support G54-G59 (WCS 1-6). Extended work offsets are not supported."));
    return;
  }
  ...
  switch (cycleType) {
  case "probing-x":
    // O9100 - Single X surface probe
    // Fusion "positive" = probe moves +X = approach FROM -X = our I=-1
    // Fusion "negative" = probe moves -X = approach FROM +X = our I=1
  ...
  case "probing-y":
    // O9101 - Single Y surface probe
    // Fusion "positive" = probe moves +Y = approach FROM -Y = our I=-1
    // Fusion "negative" = probe moves -Y = approach FROM +Y = our I=1
    protectedProbeMove(cycle, x, y, z);
    var dirI = cycle.approach1 === "positive" ? -1.0 : 1.0;
  ...
  case "probing-z":
    // O9102 - Single Z surface probe
    // Position above surface, O9102 probes downward, sets WCS Z
    protectedProbeMove(cycle, x, y, z);
  ...
  case "probing-xy-inner-corner":
    // O9110 - Inside corner, I/J set wall directions
    // I=1 wall in +X, I=-1 wall in -X, J=1 wall in +Y, J=-1 wall in -Y
    protectedProbeMove(cycle, x, y, z);
    var dirI = cycle.approach1 === "positive" ? 1.0 : -1.0;
  ...
  case "probing-xy-outer-corner":
    // O9111 - Outside corner, I/J set part directions
    protectedProbeMove(cycle, x, y, z);
    var dirI = cycle.approach1 === "positive" ? 1.0 : -1.0;
    var dirJ = cycle.approach2 === "positive" ? 1.0 : -1.0;
  ...
  case "probing-y-wall":
    error(localize("Channel/wall probing not supported. Use rectangular boss or rectangular pocket probing cycle instead."));
    return;

  default:
    error(localize("Unsupported probing cycle: " + cycleType + "."));
    return;
  }
}
// Define printProbeResults() outside writeProbeCycle()
...
function protectedProbeMove(cycle, x, y, z) {
  // Protected move to probe measurement position
  // 1. Retract to safe Z
  // 2. Move to XY position
  // 3. Plunge to measurement depth (z - cycle.depth)
  // Fusion passes z as retract height, cycle.depth as distance below z to measurement point
  if (cycle.retract) {
    zOutput.reset();
    writeBlock(gMotionModal.format(0), zOutput.format(cycle.retract));
  }
  ...
  if (typeof z === "number") {
    var probeZ = z;
    if (typeof cycle.depth === "number" && cycle.depth > 0) {
      probeZ = z - cycle.depth;
    }
    zOutput.reset();
    writeBlock(gMotionModal.format(0), zOutput.format(probeZ));
  }
}
// <<<<< INCLUDED FROM include_files/protectedProbeMove_renishaw.cpi
